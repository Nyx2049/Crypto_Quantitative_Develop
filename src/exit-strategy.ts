...
export function evaluateExitStrategy(
  ...
  const pnlPercentage =
    position.entry > 0
      ? (isLong ? price / position.entry - 1 : 1 - price / position.entry) * 100
      : null;
  const profitR =
    risk > 0
