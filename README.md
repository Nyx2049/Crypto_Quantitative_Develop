...
前一段趋势使用“截至 10 根 K线之前的 20 根 EMA99 角度”判断。参考止损使用最近 60 根已收盘 4H K线的结构极值：做多放在最低价下方 0.3%，做空放在最高价上方 0.3%。页面同时显示止损距现价的百分比，超过 5% 标记为偏远。推荐方向和止损只用于辅助筛选，仍需人工确认真实结构。
...
- EMA99 零度筛选器 1.0：只在上述 20 个合约中按 Angle10 绝对值从小到大排序。
